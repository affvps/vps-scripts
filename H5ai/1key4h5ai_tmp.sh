...
#	Version: 2.1
#	Blog: https://www.wulabing.com https://affvps.net
# ====================================================
...
basic_dependency(){
    apt update
    apt install wget curl unzip libcurl3 net-tools bc -y     
}
nginx_install(){
#        debian_source
        apt install nginx -y
...
h5ai_install(){
    mkdir -p /home/wwwroot/${domain} && cd /home/wwwroot/${domain}&& wget ${h5ai_download_http} && unzip h5ai_dplayer.zip
    if [[ $? -eq 0 ]];then
...
domain_check(){
    stty erase '^H' && read -p "请输入你的域名信息(eg:www.example.com):" domain
    ## ifconfig
